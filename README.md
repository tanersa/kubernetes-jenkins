...
Let's create now our persistent volumes:
   ...
   -  Note: With statefulset, once we scale up our PODs, its going to create Persistent Volumes but we didnt see same result with deployment. If we deploy PODs on different worker nodes, these nodes will have persistent storage for our app.
